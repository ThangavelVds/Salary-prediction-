...
{ 
    ...
    if (pid == 0) { 
        ...
        if (np > 1) { 
            for (i = 1; i < np - 1; i++) { 
                ...
                MPI_Send(&elements_per_process, 1, MPI_FLOAT, i, 0, MPI_COMM_WORLD);  
                MPI_Send(&a[index], elements_per_process, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
	        MPI_Send(&b[index],elements_per_process,MPI_FLOAT, i, 0,MPI_COMM_WORLD);	  
            } 

            index = i * elements_per_process; 
            int elements_left = n - index; 

            MPI_Send(&elements_left, 1, MPI_FLOAT, i, 0,  MPI_COMM_WORLD);         
            MPI_Send(&a[index], elements_left,  MPI_FLOAT, i, 0,  MPI_COMM_WORLD);
	    MPI_Send(&b[index],elements_per_process,MPI_FLOAT, i, 0,MPI_COMM_WORLD); 
        } 
	...
	float tmp2;
        for (i = 1; i < np; i++) 
	{ 
            MPI_Recv(&tmp1, 1, MPI_FLOAT,MPI_ANY_SOURCE,0,MPI_COMM_WORLD, &status);           
	    MPI_Recv(&tmp2, 1, MPI_FLOAT,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,&status); 
    ...
    printf("\n****************************************************************\n");
  	...
  	MPI_Recv(&b2, n_elements_recieved,MPI_INT, 0, 0,MPI_COMM_WORLD,&status); 
        ...
        for (int i = 0; i < n_elements_recieved; i++) 
	{
        ...
        MPI_Send(&partial_sum_1, 1, MPI_FLOAT,0, 0, MPI_COMM_WORLD); 
	MPI_Send(&partial_sum_2, 1, MPI_FLOAT,0, 0, MPI_COMM_WORLD);
    }    
    MPI_Finalize(); 
